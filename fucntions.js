...
function isValideExprFunctionGraph(arrayIndexObjects,expression) {
    ...
    if (!regex.test(expression) || !isValideIdObjectExpresion(arrayIndexObjects,expression)) {
        return false;
    }
    var parentheses = 0;
...
function getParamsPoint(point,bool=null){
    return ['point',
        {type:'coord', attribute: "coord", value: point !== null ? point.coords.usrCoords.slice(1) : 0, for: ((objects.length > 0 && existeObjetc(objects))) ? 'text' : 'number',readonly : ((objects.length > 0 && bool == null) || bool), display : true,win2 : true},
...
function getParamsLine(line){
    return ['line',
        {type:'coord', attribute: "point1", for: existeObjetc(objects)? 'text' :'number',win2 : true},
        {type:'coord', attribute: "point2",  for: existeObjetc(objects)? 'text' :'number',win2 : true},
...
function callFunctionCircle(circle,base,circum=null) {
    ...
    objects.push({type : !circum ? 'circle' :'circumcircle' , object: circle, base : base});
    $('#windowModal').modal('hide');
    affichageParametres(getParamsCircle(circle));
    gestionParametresCircle(circle);
}
function createPointCircle(arrayIndexObjects, centerx, centery) {
    ...
    if (($('#object-base').length == 0) || (!isNaN(centerx) && !isNaN(centery))) {
        ...
        center = [creationCoordPoint(arrayIndexObjects, centerx), creationCoordPoint(arrayIndexObjects, centery)];
        return center;
    }
...
function callFunctionAnimation(base) {    
...
function creationAnimationFactory() {
    ...
    else if (functionsCreationAnimation){
        isAnimation = false
        $('.button-animation').html(`<img src="./start.png" width="50" height="50" title="reactualise" 
...
function getPointCode(object) {
    ...
    jsCode = `const point_${object.object.id} = board.create('point', [${coordX}, ${coordY}],
        { 
            id : '${object.object.id}',
...
function getCircleCode(object,circum=null){
    ...
    if(!circum){
        ...
        jsCode = `
        circumcircle_${object.object.id} = board.create('circumcircle',[`
        typeof(coord1) === 'string' ? jsCode += `${coord1},`: jsCode +=`[${coord1}],`
        typeof(coord2) === 'string' ? jsCode += `${coord2},`: jsCode +=`[${coord2}],`
...
function getAnimationCode(obj){
    ...
    jsCode += `function startAnimation() {
        ${modifyStringAnimation(transformExprePointVisualiser(obj.base.arg1,obj.base.ids),functionsCreationAnimation ? 'an':null)}
    }`
    return jsCode;
}
...
function extractTypes(code) {
    ...
    var regex = /(board\.create|JXG\.JSXGraph\.initBoard)\('(\w+)'[^)]*\)(?:[\s\S]*?)(?=(board\.create|JXG\.JSXGraph\.initBoard)|$)/g;
    var match;
...
function extraireCreations(code) {
    const regex = /(board\.create|JXG\.JSXGraph\.initBoard)\([\s\S]+?}\);/g;
...
function replaceObjectsInString(str) {
    return str.replace(/point_p(\d+)/g, function(match, number) {
      ...
      if (index !== -1) {
        ...
        return match; // Retourne le match original si l'index n'est pas trouvé
      }
...
function trouverIdsBase(code) {
    const regex = /(point|slider|circle)_(\w+)/g;
    const matches = code.match(regex);
...
function parseBoardCode(code) {
  const regex = /board\.create\(['"](?:circle|polygon|point|axis|grid|line|text|image|circumcircle|grid|functiongraph)['"],\s*\[([\s\S]*?)\](?:,\s*\{([\s\S]*?)\})?\);/g;
  const results = [];
...
function ExpressionToCoord(str) {
    // Vérifier si la chaîne n'est pas un nombre
    if (isNaN(str)) {
      // Vérifier si la chaîne est sous la forme d'une fonction ou un simple point_p(number) ou circle_c(number)
      if (
        (str.includes("function") && str.includes("return") && str.includes(";")) ||
        /^point_p\d+$/.test(str) ||
        /^circle_c\d+$/.test(str)
      ) {
        // Extraire la partie entre "return" et ";}" pour les fonctions
        let result = "";
        ...
        // Transformer "point_p(number)"
        result = result.replace(/point_p(\d+)/g, "p$1");
  
        // Transformer "circle_c(number)"
        ...
        // Transformer ".Raduis()" en ".r"
        result = result.replace(/\.Raduis\(\)/g, ".r");
  
        // Transformer ".Value()" en ""
...
function extractBoundingBox(code) {
    const regex = /boundingbox:\s*\[([\d\s,-]+)\]/;
    const match = code.match(regex);
  
    if (match) {
      const boundingBoxString = match[1];
      const boundingBoxValues = boundingBoxString.split(',').map(value => parseFloat(value.trim()));
...
$('#importerInput').on('change', function(event) {
    ...
    fileReader.onload = function() {
        ...
        var creationObjects = extraireCreations(importedCode);
        var variablesTypes = extractTypes(importedCode);   
        variablesTypes.forEach((obj,index) => {
        switch(obj){
            case 'grid' :
                //grid = eval(creationObjects[index]);
                break;
            case 'box' :
                board.setBoundingBox(extractBoundingBox(creationObjects[index]));
                break;
            case 'axis' :
                eval(creationObjects[index]);
                break;  
            /*case 'text' :
                cmpText++
                ...
                callFunctionText(text,{ids:tabIds,arg1 :ExpressionToCoord(tabCoords[0]) , arg2:ExpressionToCoord(tabCoords[1]) , arg3 :ExpressionToCoord(tabCoords[2]).replace(/"/g, '')});
                break;
            case 'image' :
                cmpImage++;
            ...
            case 'point':
                ...
                tabIds = supprimerDoublons(trouverIdsBase(tabString[0]));
                tabCoords = parseStringList(tabString[0]);
                X = ExpressionToCoord(tabCoords[0]);
                Y = ExpressionToCoord(tabCoords[1]);
                callFunctionPoint(point,true,{ids:tabIds,coordX : X, coordY: Y});
                break;
            case 'polygon':
                var polygon = eval(replaceObjectsInString(creationObjects[index]));
                tabString = parseBoardCode(creationObjects[index]);
                tabCoords = parseStringList(tabString[0]);
                tabIds = supprimerDoublons(trouverIdsBase(tabString[0]));
                /*for (var j = 0; j < polygon.vertices.length ; j++) {
                    pointsVisualiser.push([ExpressionToCoord(tabCoords[j]),y]);
                    points.push(point);
                }                //callFunctionPoint(point,true,{ids:tabIds,coordX : X, coordY: Y});
                break; */
        }
